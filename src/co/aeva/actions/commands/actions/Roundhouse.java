...
public class Roundhouse extends PlayerAction {
	
	public Roundhouse(int cooldown) {
		super(cooldown);
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (super.onCommand(sender, cmd, label, args))
			for (Player target : affect(sender, args)) {
				...
				ParticleEffect.CRIT.display(1F, 1F, 1F, 1F, 4, target.getLocation(), 10.0D);
				target.playSound(target.getLocation(), Sound.ENTITY_PLAYER_BIG_FALL, 1, 1);
			}
		return true;
	}
	...
	public void broadcast(CommandSender pl, Player target) {
		Utils.broadcast("&b" + pl.getName() + "&c roundhouse kicked &b" + target.getName() + "&c! Ouch!");
	}
}